...
class NGPNetworks(nn.Module):
    def __init__(self, use_fully=True, density_hidden_layer=1, density_n_neurons=64, rgb_hidden_layer=2, rgb_n_neurons=64, use_sig=False):
        ...
        if self.use_fully and jt.flags.cuda_archs[0] >= 75 and self.using_fp16:
            ...
            self.density_mlp = FMLP([self.pos_encoder.out_dim] + [density_n_neurons for _ in range(density_hidden_layer)] + [16])
            self.rgb_mlp = FMLP([self.dir_encoder.out_dim+16] + [rgb_n_neurons for _ in range(rgb_hidden_layer)] + [3])
        else:
            ...
            if use_sig:
                self.density_mlp = nn.Sequential(
                    ...
                    nn.ReLU(),
                    *[nn.Linear(density_n_neurons, density_n_neurons, bias=False) if ii%2 == 0 else nn.Sigmoid() for ii in range(2*(density_hidden_layer-1))],
                ...
                self.rgb_mlp = nn.Sequential(nn.Linear(self.dir_encoder.out_dim+16, rgb_n_neurons, bias=False),
                                nn.ReLU(),
                                *[nn.Linear(rgb_n_neurons, rgb_n_neurons, bias=False) if ii%2 == 0 else nn.Sigmoid() for ii in range(2*(rgb_hidden_layer-1))],
                                nn.Linear(rgb_n_neurons, 3, bias=False))
            else:
                self.density_mlp = nn.Sequential(
                    nn.Linear(self.pos_encoder.out_dim, density_n_neurons, bias=False), 
                    nn.ReLU(),
                    *[nn.Linear(density_n_neurons, density_n_neurons, bias=False) if ii%2 == 0 else nn.ReLU() for ii in range(2*(density_hidden_layer-1))],
                    nn.Linear(density_n_neurons, 16, bias=False))
                self.rgb_mlp = nn.Sequential(nn.Linear(self.dir_encoder.out_dim+16, rgb_n_neurons, bias=False),
                                nn.ReLU(),
                                *[nn.Linear(rgb_n_neurons, rgb_n_neurons, bias=False) if ii%2 == 0 else nn.ReLU() for ii in range(2*(rgb_hidden_layer-1))],
                                nn.Linear(rgb_n_neurons, 3, bias=False))
        self.set_fp16()
