...
class Runner():
    ...
    def train(self):
        ...
        for i in tqdm(range(self.tot_train_steps)):
            ...
            if i>0 and (i%self.val_freq==0 or i == self.tot_train_steps - 1):
                psnr=self.val_img(i)
                print("STEP={} | LOSS={} | VAL PSNR={}".format(i,loss.mean().item(), psnr))

                if psnr > max_psnr:
                    # save models checkpoint
                    # os.path.join(self.save_path, "ckpts")
                    self.save_ckpt(os.path.join(self.save_path, "params.pkl"), i)
                    max_psnr = psnr
                    
        self.valid(True)
        self.test(True)

    def valid(self, load_ckpt=False):
        if load_ckpt:
        ...
        if not os.path.exists(os.path.join(self.save_path, "valid")):
            os.makedirs(os.path.join(self.save_path, "valid"))
        mse_list=self.render_valid(save_path=os.path.join(self.save_path, "valid"))
        if self.dataset["val"].have_img:
            tot_psnr=0
            for mse in mse_list:
                tot_psnr += mse2psnr(mse)
            print("TOTAL VALID PSNR===={}".format(tot_psnr/len(mse_list)))
    
    def render_valid(self, save_img=True, save_path=None):
        if save_path is None:
            save_path = self.save_path
        mse_list = []
        print("rendering validation set...\n Total:" + str(self.dataset["val"].n_images))
    ...
    def load_ckpt(self, path):
        ...
        ckpt = jt.load(path)
        print("ckpt step:", ckpt['global_step'])
    ...
    def val_img(self, iter):
        ...
        if not os.path.exists(os.path.join(self.save_path, "valid")):
            os.makedirs(os.path.join(self.save_path, "valid"))
        mse_list=self.render_valid(save_img=False, save_path=os.path.join(self.save_path, "valid"))
        if self.dataset["val"].have_img:
            tot_psnr=0
            for mse in mse_list:
                tot_psnr += mse2psnr(mse)
