...
def create_nerf(args):
    ...
    if len(ckpts) > 0:
        state_dict = jt.load(ckpts[-1])
        model.load_state_dict(state_dict['network_fn_state_dict'])
        model_fine.load_state_dict(state_dict['network_fine_state_dict'])
        print('start from training step:', state_dict['global_step'])
...
def train():
    ...
    # training logs
    tlog_path = os.path.join(basedir, expname, 'eval_log.txt')
    with open(tlog_path, 'w') as file:
        file.write("ckpts evaluation:\n")
    ...
    for i in trange(start, N_iters):
        ...
        if (i+1)%args.i_weights==0 and (not jt.mpi or jt.mpi.local_rank()==0):
            ...
            with open(tlog_path, 'a') as file:
                file.write(f"[EVAL] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}\n")
