...
1. 提出了一种新的NeRF模型：multistage nerf
2. 充分发挥各种NeRF模型的优势，基于不同场景综合效率与准确度选取最优NeRF模型
3. 提出了能够适配不同NeRF模型的增强渲染模型
4. 使用仿射变换、缩放裁剪数据增强用于NeRF以及增强渲染模型
...
该方法使用了具有两个隐层的MLP，包含64个神经元以及ReLU函数构建网络结构，损失和输出激活的选择是特定于任务的。当训练数据具有低动态范围(sRGB)时，我们使用sigmoid激活；当训练数据具有高动态范围(线性)时，使用指数激活（线性HDR)。

### realESRGAN

realESRGAN主要目标为解决真实场景下的图像模糊问题。
...
在本项目中，我们使用了VGG19完成了ImageNet的预训练。
...
1. multistage nerf
   我们实现了multistage NeRF，使用pipeline将增强渲染模型与NeRF相结合，取得了显著的效果提升。
   其算法流程为先使用训练集训练NeRF，之后将NeRF生成的低质量图片进行旋转、缩放、剪切等进行数据增强。之后使用NeRF生成的图片与旋转、缩放、剪切后的图片作为增强渲染模型的训练集训练增强渲染模型。
   最后先将测试集的数据输入NeRF，再将NeRF输出的图片输入增强渲染模型，得到最后的结果。
   其训练流程见下图：
...
3. 对于不同NeRF模型的适配
   尽管我们针对不同的场景使用了不同的NeRF模型，但是增强渲染模型却能够适配所有的NeRF。因为增强渲染模型的输入为NeRF给出的低质量图片，输出为增强渲染后的结果，且训练过程相对独立，故该模型可以适配所有的NeRF模型，该模型可以增强渲染不同NeRF模型给出的输出图片。
4. 仿射变换、缩放裁剪数据增强用于NeRF以及增强渲染模型

   我们使用了数据增强从有限的数据产生更多的数据，增加训练样本的数量以及多样性，提升了模型的鲁棒性。在本项目中，一大难题为我们难以得到充足的数据来完成任务，导致某些场景渲染结果不佳。

   我们通过仿射变换、缩放裁剪数据，**在仅使用给定训练集、测试集的前提下**，降低了模型对某些属性的依赖，从而提高模型的泛化能力，得到了较好得结果。
